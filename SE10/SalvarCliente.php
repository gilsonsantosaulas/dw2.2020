<?php 
    ...
    if($id==0) { // se o id for 0 eh um novo registro (insert)
		$sql = "insert into clientes(nome, email, foto) values (?,?,?)";
		$sqlprep = $conexao->prepare($sql);
		$sqlprep->bind_param("sds", $nome, $email, $nome_arquivo);            
		$sqlprep->execute();
		$msgOk = "Cliente inserido com sucesso.";
	} else { // ser o id nao for 0 eh um atualizacao (update)
		$sql = "update clientes set nome=?, email=?, foto=? where id=?";
		$sqlprep = $conexao->prepare($sql);
